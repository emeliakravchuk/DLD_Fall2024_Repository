...
for extra credit:
\begin{itemize}
\item Add other operations (e.g., SHA-512)
...
\item Determine a way to optimize the operation of the register to
  process throughput faster (e.g., adding another \verb!main_comp! block)
\item Determine the best timing to optimize the throughput
\item Add poster to explain to the masses!
...
\subsection{Integrated Logic Analyzer}
...
in your design.  However, usually not all signals need to be probed.
...
\item  Build the testbench to simulate your design and make sure
  ...
  \item Finally compare the timing of both designs and provide a
    justification if this project's design can process $1$ million SHA
    vectors faster or slower than Lab~$2$'s implementation.
\end{enumerate}
